"""
Test: What happens when we shuffle positions each batch?

Both position-based AND token co-occurrence patterns are broken.
The model can't learn any stable strategy.
"""
...
def generate_batch_shuffled(bs, seq_len, vocab_size=16):
    """Generate random sequence with NO echo structure — completely shuffled."""
    x = torch.randint(0, vocab_size, (bs, seq_len), device=DEVICE)
    # Target is just random tokens too — no structure at all
...
def run():
    print("=" * 60)
    print("TEST: Shuffled Positions (No Learnable Structure)")
    print("=" * 60)
    ...
    for step in range(N_BATCHES):
        x, y = generate_batch_shuffled(8, 32)
